const swiper = new Swiper(".mySwiper", {
  ...
  for (var i = 0; i < maxResult; i++) {
    ...
    let prod_red = (product.price - Math.round((product.price*20)/100))
  
    categoriesProducts.insertAdjacentHTML(
      'beforeend',
      `<div class="product">
          <div class="top d-flex">
            <img class="bucket_prod_img" src=${product.url} alt="" />
            <div class="icon d-flex">
              <i class="bx bxs-heart"></i>
            </div>
          </div>
          <div class="bottom">
            <div class="d-flex">
              <h4 class="bucket_prod_title" >${product.title}</h4>
              <button class="btn cart-btn">Ajouter</button>
            </div>
            <div class="d-flex">
              <div id="bucket_prod_price" class="price "> <b class="bucket_prod_price" > ${product.price}</b>FCFA</div>
              <div class="rating">
              <b>${prod_red}</b>
                <i"> à partir de <b>3</b></i>
              </div>
            </div>
          </div>
        </div>`
